...
public class PartTwo {
        ...
        Map<String, BigInteger> sequenceMap = new HashMap<>();
        for (int i = 0; i < polymerTemplate.length(); i++) {
            String find = polymerTemplate.substring(i, i+1);
            if(!sequenceMap.containsKey(find)){
                sequenceMap.put(find,BigInteger.ZERO);
            }
            sequenceMap.put(find, sequenceMap.get(find).add(BigInteger.ONE));
        }
        ...
        List<BigInteger> values = new ArrayList<>(sequenceMap.values());
        Collections.sort(values);

        System.out.println(values.get(values.size()-1).subtract(values.get(0)));
    }
}
